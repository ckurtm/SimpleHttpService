...
public class QueuedThreadPool extends AbstractLifeCycle implements ThreadPool.SizedThreadPool, Executor, Dumpable {
    ...
    protected void doStop() throws Exception {
        ...
        if (size > 0) {
            Log.d(getClass().getSimpleName(), size + " threads could not be stopped");

            if (size == 1) {
                for (Thread unstopped : _threads) {
                    Log.i(getClass().getSimpleName(), "Couldn't stop " + unstopped);
                    for (StackTraceElement element : unstopped.getStackTrace()) {
                        Log.d(getClass().getSimpleName(), " at " + element);
                    }
                }
            }
        }
    ...
    public boolean dispatch(Runnable job) {
        ...
        Log.d(getClass().getSimpleName(), "Dispatched {} to stopped {}");
        return false;
    }
